...
class SourceCatalogue:
    ...
    def is_live_fetch_authorized(self, source_id: str) -> bool:
        ...
        if not bool(row.get("enabled", False)):
            return False
        if bool(row.get("online_validation_required", True)):
            # Controlled-online still unauthorized in MI-EXT-001 wave 1
            if str(row.get("access_status") or "").upper() != "LIVE_AUTHORIZED":
                return False
        return str(row.get("access_status") or "").upper() == "LIVE_AUTHORIZED"
