...
from backend.intelligence.external_events.constants import DEFAULT_FRESHNESS_WINDOWS_SEC, UNKNOWN, UNAVAILABLE
...
def evaluate_freshness(
    *,
    ...
    if published is None:
        retrieved = parse_utc(retrieved_at or "")
        if retrieved is None:
            return UNKNOWN
        published = retrieved

    # Events timestamped after evaluation time T cannot be treated as fresh evidence.
    if published > now:
        return "FUTURE"
...
def is_lookahead_freshness(status: str) -> bool:
    return str(status).upper() == "FUTURE"
