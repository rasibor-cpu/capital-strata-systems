...
class OpportunityRouter:
    def __init__(self, trade_threshold: float = 0.68, watch_threshold: float = 0.55) -> None:
        self.scanner = UnifiedMarketScanner()
        self.regime_detector = MarketRegimeDetector()
        self.scorer = AIOpportunityScorer(trade_threshold=trade_threshold, watch_threshold=watch_threshold)
        self.trade_threshold = trade_threshold
        self.watch_threshold = watch_threshold
        self.opportunity_scoring_engine = OpportunityScoringEngine()
        self.allocation_intelligence_engine = AllocationIntelligenceEngine()
    ...
    def route(self, include_watchlist: bool = False) -> List[Dict[str, Any]]:
        ...
        portfolio_context = self.allocation_intelligence_engine.analyze_portfolio(raw_opportunities)
        ...
        for asset in raw_opportunities:
            ...
            scoring_payload = self._build_scoring_payload(asset=prepared, regime=str(regime_result.regime))
            ...
            candidate_payload["allocation_guidance"] = self.allocation_intelligence_engine.analyze_candidate(candidate_payload, portfolio_context).to_dict()
    ...
    def _build_scoring_payload(self, asset: Dict[str, Any], regime: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_volatility": self._first_float(raw, ["avg_volatility", "average_volatility"], 0.0),
            "regime": regime,
            "momentum": asset["trend_pct"],
            "trend_strength": abs(asset["trend_pct"]),
            "volume_24h": self._first_float(raw, ["volume_24h", "quote_volume", "base_volume", "notional_volume"], 0.0),
            "avg_volume_24h": self._first_float(raw, ["avg_volume_24h", "average_volume_24h", "avg_quote_volume"], 0.0),
            "top_of_book_depth": self._first_float(raw, ["top_of_book_depth", "book_depth", "depth", "bid_ask_depth"], 0.0),
            "slippage_bps": self._first_float(raw, ["slippage_bps", "estimated_slippage_bps"], 10.0),
            "order_flow_delta": self._first_float(raw, ["order_flow_delta", "flow_delta", "delta"], 0.0),
            "buy_pressure": self._first_float(raw, ["buy_pressure", "bid_pressure"], 0.0),
            "sell_pressure": self._first_float(raw, ["sell_pressure", "ask_pressure"], 0.0),
            "recent_high": self._first_float(raw, ["recent_high", "high", "session_high"], 0.0),
            "recent_low": self._first_float(raw, ["recent_low", "low", "session_low"], 0.0),
            "rejection_strength": self._first_float(raw, ["rejection_strength"], 0.0),
            "wick_reversal_strength": self._first_float(raw, ["wick_reversal_strength"], 0.0),
...
def print_routes(routes: List[Dict[str, Any]]) -> None:
    ...
    for r in routes:
        print(
            f"{r['symbol']} | {r['asset_class']} | decision={r['decision']} | "
            f"score={r['score']:.4f} | regime={r['regime']} | strategy={r['strategy']}"
        )
