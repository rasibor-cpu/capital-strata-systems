...
from backend.app.futures.futures_contract_registry import get_futures_contract
from backend.app.futures.futures_execution_adapter import FuturesExecutionAdapter
from backend.execution.canonical_trade_lifecycle import CanonicalTradeLifecycle, CanonicalTradeLifecycleError


class FuturesLifecycleAdapter(CanonicalTradeLifecycle):
    """Futures-specific lifecycle adapter that exposes the canonical lifecycle contract."""

    def __init__(self, repository: TradeOutcomeRepository | None = None) -> None:
        super().__init__(repository=repository)
        self.execution_adapter = FuturesExecutionAdapter()
    ...
    def execute_paper_order(self, *, symbol: str, side: str, contracts: int, mode: str) -> dict[str, Any]:
        ...
        result = self.execution_adapter.execute_futures_order(
            symbol=symbol,
            side=side,
            contracts=contracts,
            mode=mode,
