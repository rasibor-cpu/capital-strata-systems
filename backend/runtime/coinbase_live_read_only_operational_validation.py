...
class CoinbaseLiveReadOnlyOperationalValidator:
    ...
    def validate(self) -> dict[str, Any]:
        ...
        server_time, read_checks["server_time"], failure = _read(lambda: adapter.get_server_time())
        if failure:
            failures.append(failure)
        read_checks["api_connectivity"] = "OK" if server_time is not None else "FAILED"

        account, read_checks["account_retrieval"], failure = _read(lambda: adapter.get_account())
        if failure:
            failures.append(failure)

        portfolio, read_checks["portfolio_retrieval"], failure = _read(lambda: adapter.get_portfolios())
        if failure:
            failures.append(failure)

        balances, read_checks["available_balances"], failure = _read(lambda: adapter.get_balances())
        if failure:
            failures.append(failure)

        products, read_checks["products_list"], failure = _read(lambda: adapter.get_products())
        if failure:
            failures.append(failure)

        ticker, read_checks["market_ticker"], failure = _read(lambda: adapter.get_ticker())
        if failure:
            failures.append(failure)
