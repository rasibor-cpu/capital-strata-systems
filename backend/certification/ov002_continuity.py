...
def evaluate_final_certification(
    *,
    ...
    persisted_attempt_state: Mapping[str, Any] | str | None = None,
    ...
    persisted_state_value = ""
    if isinstance(persisted_attempt_state, Mapping):
        persisted_state_value = str(
            persisted_attempt_state.get("attempt_state")
            or persisted_attempt_state.get("state")
            or ""
        ).upper()
    elif persisted_attempt_state is not None:
        persisted_state_value = str(persisted_attempt_state).upper()
    run_status_attempt_state = str(run_status.get("attempt_state") or "").upper()

    if (
        invalidation
        or str(run_status.get("status") or "").upper() == STATE_INVALIDATED
        or persisted_state_value == STATE_INVALIDATED
        or run_status_attempt_state == STATE_INVALIDATED
    ):
        reasons.append("attempt_invalidated")
        if persisted_state_value == STATE_INVALIDATED:
            reasons.append("persisted_attempt_state_invalidated")
