...
# ===============================
# EXECUTION ENGINE (RANKED SIZING)
# ===============================
...
class ExecutionEngine:
    ...
    def get_ranked_edge_strength(self, trade):

        edges = [t.net_edge for t in self.current_trades if t.asset_class == trade.asset_class]
        ...
        return (trade.net_edge - min_edge) / (max_edge - min_edge)
    ...
    def regime_factor(self, state, trade):
        ...
        if state == "STRONG":
            ...
            edge_strength = self.get_ranked_edge_strength(trade)
...
class Dashboard:
    ...
    def run(self):
        ...
        print("\nCSS PHASE 12.2 — RANKED SIZING ACTIVE\n")

        while True:

            trades = scan_market()
            self.exec.set_current_trades(trades)
