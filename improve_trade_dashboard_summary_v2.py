...
backup = Path("scripts/css_live_dashboard.py.bak_before_dynamic_trade_summary_v2")

text = p.read_text(encoding="utf-8")
shutil.copy2(p, backup)

start = text.find("def render_trade_dashboard_summary() -> None:")
end = text.find("def r17_execute_exit(pos, observer_symbol, observer_price, reason):", start)

if start == -1:
    raise RuntimeError("render_trade_dashboard_summary function not found.")
if end == -1:
    raise RuntimeError("r17_execute_exit anchor not found.")
