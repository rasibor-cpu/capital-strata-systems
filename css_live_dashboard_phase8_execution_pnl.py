...
ASSET_RULES = {
    ...
    "OPTIONS": {"min_score": 0.35, "min_edge": 0.008},
}
...
def build_option_candidates(base_trades: List[Trade]) -> List[Trade]:
    """
    Derive synthetic options candidates from strong underlyings.
    Safe mode only: no live options chain dependency yet.
    """

    option_bases = []
    for trade in base_trades:
        if trade.asset_class in {"CRYPTO", "FUTURES"}:
            option_bases.append(trade)
    ...
    for trade in option_bases:
        ...
        option_symbol = f"{trade.symbol}_{side}"

        # options amplify directional conviction but have higher friction
        option_score = min(1.0, trade.score * 1.10)
        option_raw = min(1.0, trade.raw_score * 1.10)
        option_move = min(0.25, trade.expected_move * 1.40)
...
def scan_market() -> List[Trade]:
    ...
    option_trades = build_option_candidates(trades)
...
class Dashboard:
    ...
    def run_cycle(self):
        ...
        for trade in trades:
            ...
            if self.edge_gate.passes(trade):
                ...
                asset_count = class_counts.get(trade.asset_class, 0)
                asset_cap = MAX_PER_ASSET.get(trade.asset_class, 1)

                if len(selected_trades) < MAX_SELECTED and asset_count < asset_cap:
                    if is_elite:
                    ...
                    class_counts[trade.asset_class] = asset_count + 1
                else:
    ...
    def run(self):
        ...
        print("\nCSS PHASE 8 — EXECUTION + PNL + OPTIONS MIX MODE STARTED\n")
