...
def force_console_recovery_setup(users: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    render_console_auth_status(
        ...
        "Answer all five recovery questions before CSS access can continue.",
    )
    ...
    for index, question in enumerate(RECOVERY_QUESTIONS, start=1):
        ...
        while True:
            ...
            if answer != confirm:
                render_console_auth_status("RECOVERY ERROR", "Recovery answers do not match.")
                continue
            if not hash_recovery_answer(answer):
                render_console_auth_status("RECOVERY ERROR", "Recovery answer cannot be blank.")
                continue
    ...
    render_console_auth_status(
        "RECOVERY CONFIGURED",
        "All five password recovery answers are configured successfully.",
    )
...
def await_gui_login(users: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def show_required_recovery_setup(user_id: str) -> None:
        ...
        tk.Label(
            content,
            text="Answer all five recovery questions before CSS access can continue. Answers are masked and stored as hashes.",
            bg=colors["panel"],
            fg=colors["muted"],
            font=subtitle_font,
            wraplength=520,
        ...
        for index, question in enumerate(RECOVERY_QUESTIONS, start=1):
            ...
            answer_entry.grid(row=row_index + 1, column=0, sticky="ew", ipady=8, pady=(4, 10))
            answer_vars.append((question, answer_var, answer_entry))
            row_index += 2

        def submit_required_recovery() -> None:
            answers = {question: var.get() for question, var, _entry in answer_vars}
            try:
                enroll_all_password_recovery(active_users, user_id, answers)
                save_users(active_users)
            except PasswordValidationError as exc:
                set_status(str(exc), "error")
                return
            ...
            set_status("All five password recovery answers configured.", "success")
            finish(user_ctx)

        button_row = tk.Frame(content, bg=colors["panel"])
        button_row.grid(row=row_index, column=0, sticky="ew", pady=(18, 18))
        primary_button(button_row, "Save All 5 Answers and Continue", submit_required_recovery).pack(side="left")
        secondary_button(button_row, "Cancel", root.destroy).pack(side="left", padx=(12, 0))

        attach_status(row_index + 1)
        set_status("Recovery setup is mandatory for this account.", "info")
        answer_vars[0][2].focus_set()
