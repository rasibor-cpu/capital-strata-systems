...
BTC Trend Pullback Backtest v25

Fixes v24 bug:
Correct pullback detection using state memory.

Strategy logic
1) Strong trend (price > EMA50 and EMA50 slope positive)
2) Pullback occurs (price drops below EMA20)
3) Continuation (price crosses back above EMA20)
4) ATR-based stop
5) Partial profit at 2R
6) Trailing stop
"""
...
def fetch(product,start,end,gran):
    ...
    while cursor<end:
        ...
        for row in r.json():
            ...
            candles.append(
                Candle(int(ts),float(low),float(high),float(open_),float(close),float(vol))
            )
...
def main():
    ...
    print("\nCSS TREND PULLBACK BACKTEST v25\n")

    print(res)

    stamp=datetime.now().strftime("%Y%m%d_%H%M%S")

    out=OUT/f"trend_pullback_backtest_v25_{stamp}.json"
