...
Autonomous Trading Engine v42

New in v42
----------
• Minimum asset price filter
• Maximum token size cap
• Maximum capital per trade cap
• Improved opportunity ranking stability
...
def discover_markets():
    ...
    return markets[:MAX_DISCOVERED_TO_RANK]
...
def fetch(product, start, end):
    ...
    while cursor < end:
        ...
        for row in r.json():
            ...
            candles.append(
                Candle(ts, low, high, open_, close, vol)
            )
...
def score_asset(asset):
    ...
    vwap_spread = abs(price - v) / v
    momentum = abs(price - closes[-20]) / closes[-20]
    volatility = atr_val / price

    score = vwap_spread + momentum + volatility
...
def run_cycle():
    ...
    print("\n==============================")
    print("CSS AUTONOMOUS ENGINE v42")
...
def main():
    ...
    print("\nCSS AUTONOMOUS ENGINE v42 STARTED\n")
