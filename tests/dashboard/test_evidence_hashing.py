...
from dashboard.runtime.api_bridge import create_app, get_evidence_hash_chain_payload
...
def test_evidence_hash_chain_api_route_is_read_only() -> None:
    app = create_app()
    routes = {getattr(route, "path", "") for route in app.routes}
    payload = get_evidence_hash_chain_payload()

    assert "/api/v1/evidence-hash-chain" in routes
    ...
    assert all(item["evidence_hash_id"].startswith("EVHASH-") for item in payload["evidence_items"])


def test_evidence_hash_chain_ui_rendering() -> None:
    app = create_web_app()
    routes = {getattr(route, "path", "") for route in app.routes}
    markup = _micro_live_pilot_readiness_page()

    assert "/micro-live-pilot-readiness" in routes
    assert "Evidence Integrity Hash" in markup
    assert "SHA-256" in markup
    assert "/api/v1/evidence-hash-chain" in markup
