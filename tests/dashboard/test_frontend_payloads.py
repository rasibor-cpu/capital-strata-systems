...
def test_api_bridge_routes_are_read_only_and_dashboard_state_fed() -> None:
    ...
    required_routes = {
        ...
        "/api/v1/post-pilot-archive-manifest-hash",
    ...
    assert (
        get_micro_live_pre_pilot_go_no_go_payload(lambda: state)["trading_armed"]
        is False
    )
    assert (
    ...
    manifest_hash = get_post_pilot_archive_manifest_hash_payload(lambda: state)
    assert manifest_hash["algorithm"] == "sha256"
    assert manifest_hash["archive_write_performed"] is False
    assert manifest_hash["trading_armed"] is False
    ...
    notarization_readiness = get_evidence_notarization_readiness_payload(lambda: state)
    assert notarization_readiness["notarization_status"] == "NOT_NOTARIZED"
    assert notarization_readiness["manual_notarization_review_required"] is True
    assert notarization_readiness["notarization_provider_selected"] is False
    assert notarization_readiness["notarization_receipt_present"] is False
    assert notarization_readiness["notarization_file_written"] is False
    assert notarization_readiness["signing_key_present"] is False
    assert notarization_readiness["signing_key_exposed"] is False
    assert notarization_readiness["trading_armed"] is False
    ...
    verification_export = get_evidence_verification_checklist_export_payload(
        lambda: state
    )
    assert verification_export["checklist_status"] == verification_checklist[
        "checklist_status"
    ]
    assert verification_export["manual_verification_required"] is True
    assert verification_export["manual_verification_recorded"] is False
    assert verification_export["archive_read_performed"] is False
    assert verification_export["external_file_read_performed"] is False
    assert verification_export["verification_performed"] is False
    assert verification_export["signature_verified"] is False
    assert verification_export["notarization_verified"] is False
    assert verification_export["trading_armed"] is False
    assert verification_export["execution_allowed"] is False
    assert verification_export["broker_mutation_allowed"] is False
    assert verification_export["persistence_enabled"] is False
