...
def test_persistence_service_registers_invoice_identity_repository(
    monkeypatch,
):
    ...
    try:
        ...
        assert isinstance(
            service.invoice_identity_allocations,
            InvoiceIdentityRepository,
        )
        ...
        assert (
            health["repositories"]["invoice_identity_allocations"]
            is True
        )
    finally:
        connection.close()
...
def test_migration_014_is_applied_by_persistence_service(
    monkeypatch,
):
    connection = _isolated_connection()
    _patch_connection(monkeypatch, connection)
    ...
    try:
        ...
        table_row = connection.execute(
            ...
            FROM sqlite_master
            WHERE type = 'table'
              AND name = 'commercial_invoice_identity_allocations'
            """
        ).fetchone()

        assert table_row is not None
        assert table_row["name"] == (
            "commercial_invoice_identity_allocations"
        )
        ...
        migration_row = connection.execute(
            ...
            WHERE version = ?
            """,
            ("014_invoice_identity",),
        ).fetchone()

        assert migration_row is not None
        assert migration_row["version"] == "014_invoice_identity"
    finally:
        connection.close()
...
def test_foreign_keys_survive_service_migrations(
    ...
    try:
        ...
        candidate_cols = {
            row["from"]: row
            for row in fk_rows
            if row["table"] == "commercial_invoice_candidates"
        }
        profile_cols = {
            row["from"]: row
            for row in fk_rows
        ...
        for row in candidate_cols.values():
...
def test_candidate_period_and_invoice_number_uniqueness(
    monkeypatch,
):
    connection = _isolated_connection()
    _patch_connection(monkeypatch, connection)
    ...
    try:
        ...
        unique_indexes = [
            row for row in index_rows if row["unique"] == 1
        ]
        unique_col_sets = []
        ...
        assert ("policy_id", "period_start", "period_end") in (
            unique_col_sets
        ) or any(
            set(cols)
        ...
        assert any(
            cols == ("invoice_number",) or set(cols) == {"invoice_number"}
            for cols in unique_col_sets
        )
    finally:
        connection.close()
...
def test_invoice_identity_repository_remains_insert_only_through_service(
    monkeypatch,
):
    connection = _isolated_connection()
    _patch_connection(monkeypatch, connection)
    ...
    try:
        service = PersistenceService()
        repo = service.invoice_identity_allocations

        assert hasattr(repo, "create_allocation")
        assert hasattr(repo, "get_by_invoice_id")
        assert hasattr(repo, "get_by_candidate_period")
        ...
        assert not hasattr(repo, "update_allocation")
        assert not hasattr(repo, "delete_allocation")
...
def test_invoice_identity_table_has_no_prohibited_columns(
    monkeypatch,
):
    connection = _isolated_connection()
    _patch_connection(monkeypatch, connection)
