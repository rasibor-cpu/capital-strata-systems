...
def test_phase153h_state_machine_progresses_only_from_explicit_evidence() -> None:
    ...
    assert LIVE_READINESS_STATES[0] == "NOT_INITIALIZED"
    assert evaluate_live_readiness_state(base).readiness_state == "NOT_INITIALIZED"
    assert evaluate_live_readiness_state({**base, "credential_status": "PRESENT", "client_created": False}).readiness_state == "CREDENTIALS_PRESENT"
    assert evaluate_live_readiness_state({**base, "credential_status": "PRESENT"}).readiness_state == "CLIENT_CREATED"
    assert evaluate_live_readiness_state({**base, "credential_status": "PRESENT", "broker_connected": True}).readiness_state == "TRANSPORT_CONNECTED"
    # Authentication becomes PASS only after authenticated account evidence exists
    assert evaluate_live_readiness_state({**base, "credential_status": "PRESENT", "broker_connected": True, "broker_authenticated": True}).readiness_state == "TRANSPORT_CONNECTED"
    
    # Account accessible and data available
    assert evaluate_live_readiness_state({**base, "credential_status": "PRESENT", "broker_connected": True, "broker_authenticated": True, "account_equity": 20.0}).readiness_state == "ACCOUNT_DATA_AVAILABLE"
...
def test_phase153h_read_only_ready_requires_execution_disabled_and_pilot_disarmed() -> None:
    ...
    assert ready_state["readiness_state"] == "FULLY_OPERATIONAL"
    assert ready_state["go_no_go"] == "GO"
    assert armed_state["readiness_state"] == "MARKET_DATA_AVAILABLE"
...
def test_phase153h_state_machine_publishes_canonical_artifact(tmp_path: Path) -> None:
    ...
    assert published["readiness_state"] == "FULLY_OPERATIONAL"
