...
from backend.app.risk.live_microstructure_provider import DEFAULT_LIVE_MICROSTRUCTURE_PROVIDER
...
PROVIDER_DIR = REPO / "backend" / "app" / "market" / "providers"
...
def test_execution_gate_dependency_injection(tmp_path) -> None:
    market = OandaFixtureMarketProvider(FIX / "oanda_eurusd_valid.json")
    snap = market.get_snapshot(symbol="EUR_USD", context=_ctx())
    fx = FixtureFXConversionProvider(FIX / "fx_rates_valid.json")
    conv = fx.get_conversion(base_currency="USD", quote_currency="CAD", context=_ctx())
    fee = FixtureFeeModelProvider(FIX / "fee_model.json")
    slip = FixtureSlippageProvider(FIX / "slippage_model.json")
    composite = OfflineCertificationMicrostructureProvider(
        market_snapshot_provider=market,
    ...
    detailed = composite.provide_detailed(
        symbol="EUR_USD",
        side="BUY",
        notional=20.0,
        context={**_ctx(), "expected_move_bps": 50.0},
    )
    assert detailed.inputs is not None
    gate = ExecutionGate(
    ...
    result = gate.evaluate_trade(
        ...
        expected_move_bps=detailed.inputs.expected_move_bps,
        fee_bps=detailed.inputs.fee_bps,
        spread_bps=detailed.inputs.spread_bps,
        slippage_bps=detailed.inputs.slippage_bps,
        price=snap.mid,
        ...
        fx_conversion=conv,
        offline_provider_diagnostics={
            **dict(detailed.diagnostics),
            "fx_conversion_path": list(conv.conversion_path),
            "fx_path_type": conv.path_type,
            "fx_evidence_hash": conv.evidence_hash,
        },
    )
    assert result["decision"]["final"] == "ALLOW"
    assert result["debug"]["fx_conversion"]["path_type"] == "DIRECT"
...
def test_antibleed_still_fail_closed_without_certified_inputs(tmp_path) -> None:
    gate = ExecutionGate(
...
def test_phase152a_ceiling_and_authority_unchanged() -> None:
    assert float(DEFAULT_ORDER_LIMIT_CONFIG.live_pilot_max_position_cad) == 20.0
    assert any(k == "anti_bleed_guard_pass" for k, _ in AUTHORITY_CONDITIONS)
    auth = evaluate_live_execution_authority({"operator_requested_live": True, "go_no_go": "NO GO"})
    assert auth.can_live_execute is False
