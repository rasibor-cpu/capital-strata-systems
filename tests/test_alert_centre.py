"""
Tests for Alert Centre View (EWP-3 PART C)
"""
...
def test_alert_centre_categorisation(setup_platform):
    # Verify default state
    view = setup_platform.get_alert_centre_view()
    assert len(view["unread_alerts"]) == 0
    assert len(view["critical_alerts"]) == 0
    assert view["queue_status"]["is_empty"] is True
    ...
    notif_event = Event(
        ...
        category="SYSTEM",
        source="test",
        payload={"title": "Test Alert", "message": "CRITICAL issue detected", "delivery_status": "PENDING"}
    )
    setup_platform.read_model.notification_service.queue.append(notif_event)

    view_with_queued = setup_platform.get_alert_centre_view()
    assert len(view_with_queued["unread_alerts"]) == 1
    assert view_with_queued["unread_alerts"][0]["title"] == "Test Alert"
    assert view_with_queued["queue_status"]["depth"] == 1
    assert view_with_queued["queue_status"]["is_empty"] is False

    # Move alert to history (with delivery outcome)
    notif_event.payload["delivery_status"] = "SENT"
    setup_platform.read_model.notification_service.queue.clear()
    setup_platform.read_model.notification_service.history.append(notif_event)

    view_with_history = setup_platform.get_alert_centre_view()
    assert len(view_with_history["unread_alerts"]) == 0
    assert len(view_with_history["critical_alerts"]) == 1
    assert view_with_history["critical_alerts"][0]["title"] == "Test Alert"
    assert view_with_history["queue_status"]["depth"] == 0
