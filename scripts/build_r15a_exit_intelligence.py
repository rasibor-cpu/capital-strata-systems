...
OUTPUT_FILE = Path("scripts/css_live_dashboard_R15A_EXIT_INTELLIGENCE.py")
...
def evaluate_exit_signal(position: dict) -> str:
    """
    Returns:
    HOLD / TAKE_PROFIT / STOP_LOSS / RUNNER
    """
    ...
    # Simple thresholds (will refine later)
    if pnl_pct >= 0.015:
        return "TAKE_PROFIT"
...
def inject_into_position_loop(text: str) -> str:
    """
    Hook into observer position evaluation section
    """

    target = "pnl_observer.add_position"
    ...
    for line in lines:
        ...
        if target in line and not inserted:
            out.append("    exit_signal = evaluate_exit_signal(position)")
            out.append('    print(f"[R15A EXIT] {symbol} signal={exit_signal}")')
            inserted = True
...
def main():
    ...
    print("[SUCCESS] R15A EXIT INTELLIGENCE FILE CREATED")
