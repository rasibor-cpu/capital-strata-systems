...
$StdOutPath = Join-Path $RepoRoot "runtime\logs\css_canonical_runtime.out.log"
$StdErrPath = Join-Path $RepoRoot "runtime\logs\css_canonical_runtime.err.log"
$BootstrapLogPath = Join-Path $RepoRoot "runtime\logs\css_canonical_runtime.bootstrap.log"
...
# Detect canonical launcher processes. Windows venv execution may produce a
# direct parent/child interpreter shim pair with the same canonical target.
$canonicalRaw = @(
...
if ($canonicalRaw.Count -gt 1) {
    ...
    foreach ($child in $canonicalRaw) {
        $childParentPid = [int]$child.ParentProcessId
        if (-not $byPid.ContainsKey($childParentPid)) {
            continue
        }

        $parent = $byPid[$childParentPid]
        ...
        $sameScriptTarget =
            (
                ...
                $childCmd -match 'launcher[\\/]css_runtime_launcher\.py'
            ) -or
            (
    ...
    if ($shimPids.Count -gt 0) {
        $canonical = @(
            $canonicalRaw |
            Where-Object { -not $shimPids.Contains([int]$_.ProcessId) }
        )
        $collapsed = (@($shimPids) | Sort-Object) -join ", "
        Write-Host "       Collapsed interpreter shim PID(s): $collapsed"
    }
}
...
if ($canonical.Count -eq 1) {
    ...
    $pids = ($canonical.ProcessId | Sort-Object) -join ", "
    Fail "Multiple independent canonical runtime owners are running: $pids"
}
...
if (-not (Test-Path $StateFile)) {
    ...
    try {
        ...
        if ($status -eq "RUNNING") {
            Pass "Supervisor status is RUNNING"
        } else {
            Fail "Supervisor status is $status"
        }
...
if ($Failures.Count -gt 0) {
    if (Test-Path $BootstrapLogPath) {
        ...
        Get-Content -Path $BootstrapLogPath -Tail 80 -ErrorAction SilentlyContinue
    }
    ...
    if (Test-Path $StdOutPath) {
        ...
        Get-Content -Path $StdOutPath -Tail 80 -ErrorAction SilentlyContinue
    }

    if (Test-Path $StdErrPath) {
        Write-Host ""
        Write-Host "--- Latest canonical runtime stderr ---"
        Get-Content -Path $StdErrPath -Tail 80 -ErrorAction SilentlyContinue
    }
}
