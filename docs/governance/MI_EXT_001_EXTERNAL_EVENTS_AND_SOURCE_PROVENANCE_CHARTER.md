...
> **MI-EXT-001 R2 recovery addendum (canonical maintenance)**  
> Recovered onto `css-v1.0.1-maintenance` @ `e0676ce896b2eae682946e3619994d1dc0300da6` as an advisory-only overlay for the current TAI-001/TAI-002 ranking architecture. Historical RC-LIVE commit `3c7a6b61` is a reference only — it was not merged or cherry-picked. Live network ingestion remains unauthorized. Fixture/offline sources only. External events cannot grant execution authority, override Unified Trade Gate, or inflate rank from stale/future/duplicate evidence.
