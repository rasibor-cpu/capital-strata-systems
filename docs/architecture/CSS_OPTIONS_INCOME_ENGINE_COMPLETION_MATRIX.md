...
| Position management | `backend/options/options_position_manager.py` opens and updates long option positions, computes unrealized/realized PnL, supports stop loss, take profit, trailing stop, time exits, and closed logs. `backend/options/paper_income_lifecycle.py` adds paper-only short-premium income lifecycle state for OI-003 candidates. | Long-option paper position management exists. OI-004 adds paper short-option lifecycle, premium accounting, collateral reservation/release, expiration, assignment simulation, and completion. It does not execute assignments, exercise instructions, rolls, or broker actions. |
...
| Premium/PnL | PARTIAL | `backend/options/options_position_manager.py`; `backend/options/premium_accounting.py`; `backend/options/paper_income_lifecycle.py`; `backend/options/option_payoff_engine.py`; `docs/options_dashboard_pnl_spec.md` | Lifecycle persistence tests, dashboard PnL specs, and `tests/test_oi004_paper_income_lifecycle.py` | Paper lifecycle adapter persists closed long-option outcomes; OI-004 persists paper income lifecycle outcomes | None | General PnL and options exposure surfaces | PnL spec and OI-004 governance exist | Long-option and paper income premium accounting exist; broker/live income PnL and dashboard activation remain missing |
...
| Covered Calls | COMPLETE_PENDING_CERTIFICATION: OI-003 scans and ranks covered-call candidates deterministically | COMPLETE_PENDING_CERTIFICATION: paper-safe scanner selects canonical contracts and OI-002 builder validates share coverage | PARTIAL: OI-004 creates paper positions from accepted candidates only | COMPLETE_PENDING_CERTIFICATION: OI-004 paper state machine, collateral, premium accounting, expiration, and completion | PARTIAL: deterministic paper assignment simulation only; assignment execution is not present | NOT_PRESENT | COMPLETE_PENDING_CERTIFICATION: share coverage, payoff, downside, capped upside, ranking, lifecycle, and advisory flags are tested | PLACEHOLDER: no dashboard/reporting activation | COMPLETE_PENDING_CERTIFICATION: `tests/test_oi002_income_strategy_domain_model.py`; `tests/test_oi003_income_opportunity_scanner.py`; `tests/test_oi004_paper_income_lifecycle.py` |
| Cash-Secured Puts | COMPLETE_PENDING_CERTIFICATION: OI-003 scans and ranks CSP candidates deterministically | COMPLETE_PENDING_CERTIFICATION: paper-safe scanner selects canonical contracts and OI-002 builder validates cash collateral | PARTIAL: OI-004 creates paper positions from accepted candidates only | COMPLETE_PENDING_CERTIFICATION: OI-004 paper state machine, collateral, premium accounting, expiration, and completion | PARTIAL: deterministic paper assignment simulation only; assignment execution is not present | NOT_PRESENT | COMPLETE_PENDING_CERTIFICATION: cash collateral, payoff, downside, assignment cost basis, ranking, lifecycle, and advisory flags are tested | PLACEHOLDER: no dashboard/reporting activation | COMPLETE_PENDING_CERTIFICATION: `tests/test_oi002_income_strategy_domain_model.py`; `tests/test_oi003_income_opportunity_scanner.py`; `tests/test_oi004_paper_income_lifecycle.py` |
...
| Covered calls/CSP/Wheel | PARTIAL: covered calls and CSP paper lifecycle exist; Wheel missing | Missing portfolio integration | PARTIAL: OI-004 paper lifecycle for accepted CC/CSP candidates only | Missing options-specific broker authority | Missing | Missing | Missing | Placeholder only | Missing runtime reporting |
...
- Covered call strategy builder with stock/share coverage validation. OI-002 provides the paper-safe domain model, OI-003 provides the paper-safe scanner, and OI-004 provides paper lifecycle. Runtime integration remains outstanding.
- Cash-secured put strategy builder with cash collateral validation. OI-002 provides the paper-safe domain model, OI-003 provides the paper-safe scanner, and OI-004 provides paper lifecycle. Runtime integration remains outstanding.
...
2. The blockers to a complete institutional Options Income Engine are options-specific risk governance, broker state authority, evidence/journal/runtime-event integration, dashboard/reporting for income workflows, and production certification. Existing files support directional long options, debit-spread math, Greeks, paper lifecycle, display surfaces, OI-002 covered-call/cash-secured-put domain models, OI-003 income opportunity scanning, and OI-004 paper income lifecycle; they do not implement Wheel, assignment execution, rolling, live broker options integration, or income runtime activation.

3. Phase OI-004 completed deterministic paper-safe income lifecycle tracking for covered calls and cash-secured puts, without broker changes or live execution. The next smallest implementation phase is OI-005 options income risk governance.
