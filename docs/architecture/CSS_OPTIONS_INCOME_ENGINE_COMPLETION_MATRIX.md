...
| Strategy selection | `backend/options/options_strategy_engine.py` builds `LONG_CALL`, `LONG_PUT`, `CALL_DEBIT_SPREAD`, and `PUT_DEBIT_SPREAD`; `backend/options/options_income_strategy_domain.py` defines paper-safe `COVERED_CALL` and `CASH_SECURED_PUT` domain builders. | Directional long options, debit spreads, and OI-002 income strategy domain models are implemented. Income opportunity scanning and lifecycle are not implemented. |
| Strategy intelligence | `backend/options/options_intelligence_engine.py` validates candidate rows by premium, DTE, delta, volume, open interest, spread, liquidity, and score for directional/debit-spread strategies; `backend/options/options_income_opportunity_scanner.py` scans covered-call and cash-secured-put income candidates using canonical contracts and OI-002 builders. | Opportunity selection exists for directional/debit-spread strategies and OI-003 paper-safe income strategy candidates. Lifecycle and runtime activation are not implemented. |
...
| Tests | `tests/test_canonical_derivatives_foundation.py`, `tests/test_options_lifecycle.py`, `tests/test_options_greeks_data_model.py`, `tests/test_options_greeks_dashboard.py`, `tests/test_portfolio_greeks_aggregation.py`, `tests/test_options_strategy_classification.py`, `tests/test_unified_execution_pipeline.py`, `tests/test_canonical_trading_universe.py`, `tests/test_asset_lifecycle_integration.py`, `tests/test_oi002_income_strategy_domain_model.py`, `tests/test_oi003_income_opportunity_scanner.py`, and related dashboard/portfolio tests. | Tests cover canonical models, Greeks, paper lifecycle, dashboard aggregation, strategy classification, universe metadata, paper-safe execution, OI-002 income strategy domain models, and OI-003 income opportunity scanning. They do not cover live options brokerage. |
...
| Covered calls | COMPLETE_PENDING_CERTIFICATION | `backend/options/options_income_strategy_domain.py`; `backend/options/options_income_opportunity_scanner.py`; `backend/options/option_payoff_engine.py`; `backend/options/option_risk_profile_engine.py` | `tests/test_oi002_income_strategy_domain_model.py`; `tests/test_oi003_income_opportunity_scanner.py` | Domain/payoff/risk model and paper-safe scanner only; no lifecycle orchestration | None | Placeholder only | OI-002 and OI-003 governance docs | Paper-safe domain model and opportunity scanner exist; no lifecycle, assignment execution, roll, broker, or dashboard activation |
...
| Covered Calls | COMPLETE_PENDING_CERTIFICATION: OI-003 scans and ranks covered-call candidates deterministically | COMPLETE_PENDING_CERTIFICATION: paper-safe scanner selects canonical contracts and OI-002 builder validates share coverage | NOT_PRESENT | NOT_PRESENT | NOT_PRESENT: assignment exposure is modeled, assignment execution is not | NOT_PRESENT | COMPLETE_PENDING_CERTIFICATION: share coverage, payoff, downside, capped upside, ranking, and advisory flags are tested | PLACEHOLDER: no dashboard/reporting activation | COMPLETE_PENDING_CERTIFICATION: `tests/test_oi002_income_strategy_domain_model.py`; `tests/test_oi003_income_opportunity_scanner.py` |
| Cash-Secured Puts | COMPLETE_PENDING_CERTIFICATION: OI-003 scans and ranks CSP candidates deterministically | COMPLETE_PENDING_CERTIFICATION: paper-safe scanner selects canonical contracts and OI-002 builder validates cash collateral | NOT_PRESENT | NOT_PRESENT | NOT_PRESENT: assignment exposure is modeled, assignment execution is not | NOT_PRESENT | COMPLETE_PENDING_CERTIFICATION: cash collateral, payoff, downside, assignment cost basis, ranking, and advisory flags are tested | PLACEHOLDER: no dashboard/reporting activation | COMPLETE_PENDING_CERTIFICATION: `tests/test_oi002_income_strategy_domain_model.py`; `tests/test_oi003_income_opportunity_scanner.py` |
...
- Covered call strategy builder with stock/share coverage validation. OI-002 provides the paper-safe domain model and OI-003 provides the paper-safe scanner; lifecycle and runtime integration remain outstanding.
- Cash-secured put strategy builder with cash collateral validation. OI-002 provides the paper-safe domain model and OI-003 provides the paper-safe scanner; lifecycle and runtime integration remain outstanding.
...
2. The blockers to a complete institutional Options Income Engine are collateral and assignment-aware lifecycle, options-specific risk governance, broker state authority, evidence/journal/runtime-event integration, dashboard/reporting for income workflows, and production certification. Existing files support directional long options, debit-spread math, Greeks, paper lifecycle, display surfaces, OI-002 covered-call/cash-secured-put domain models, and OI-003 income opportunity scanning; they do not implement Wheel, assignment execution, rolling, live broker options integration, or income runtime activation.

3. Phase OI-003 completed deterministic paper-safe income opportunity scanning and contract selection for covered calls and cash-secured puts, without broker changes or live execution. The next smallest implementation phase is OI-004 paper income lifecycle.
