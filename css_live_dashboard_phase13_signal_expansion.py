...
def extract_features(opp):
    ...
    # 🔥 NEW: acceleration (change in momentum)
    accel = abs(m) * (1 + abs(m) * 2)

    # 🔥 NEW: volatility expansion boost
    vol_boost = v * (1 + v * 5)

    # 🔥 NEW: asymmetric move calculation
    move = max(
        0.01,
        min(
            0.25,
...
def amplify_edge(edge, score):
    ...
    # 🔥 asymmetry boost (reward strong edges more)
    if convex > 0.01:
        convex *= 1.3
...
class Dashboard:
    ...
    def run(self):
        ...
        print("\nCSS PHASE 13 — SIGNAL EXPANSION ACTIVE\n")
        ...
        while True:
            ...
            for asset in RESERVED_ORDER:
                ...
                raw_sizes = [(t, self.executor.base_size(self.balance, t, regime)) for t in selected]
